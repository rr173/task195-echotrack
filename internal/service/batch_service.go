...
func (b *BatchService) IngestWindow(ctx context.Context, w ingest.WindowInput) (*model.Batch, *ingest.IngestReceipt, error) {
	...
	if inserted {
		// 推进批次游标。
		nextCursor := model.AdvanceWindowCursor(batch.WindowCursor, w.SeqNo)
		if nextCursor > batch.WindowCursor {
			_, _ = b.store.UpdateBatchState(ctx, batch.ID, batch.Status, nextCursor)
		}
	}
