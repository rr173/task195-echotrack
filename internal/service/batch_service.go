...
// IngestWindowForBatch enforces the batch identity selected by the HTTP route.
func (b *BatchService) IngestWindowForBatch(ctx context.Context, pathBatchID string, w ingest.WindowInput) (*model.Batch, *ingest.IngestReceipt, error) {
	return b.IngestWindow(ctx, w.WithBatchID(pathBatchID))
}
