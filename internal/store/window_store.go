...
func (s *Store) UpsertWindow(ctx context.Context, w *model.Window) (ingest.IngestReceipt, bool, error) {
	...
	if err == nil {
		...
		return ingest.NewReceipt(existing.ID, existing.Checksum, false, true), false, nil
	}
	...
	result, err := s.db.ExecContext(ctx,
		`INSERT OR IGNORE INTO windows (id, batch_id, element_no, seq_no, i_json, q_json, sample_rate, status, checksum, created_at)
		 VALUES (?,?,?,?,?,?,?,?,?,?)
	...
	return ingest.NewReceipt(w.ID, w.Checksum, true, false), true, nil
}
